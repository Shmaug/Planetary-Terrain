...
namespace Planetary_Terrain {
    ...
    static class Noise {
        public static double Simplex(Vector3d position) {
            return Planetary_Terrain.Simplex.Generate(position.X, position.Y, position.Z);
        }

        public static double SmoothSimplex(Vector3d position, int octaves, double frequency, double persistence) {
            double total = 0f;
            double maxAmplifier = 0f;
            double amplitude = 1f;
            for (int i = 0; i < octaves; i++) {
                total += Simplex(position * frequency * amplitude);

                frequency *= 2f;
                maxAmplifier += amplitude;
        ...
        public static double Fractal(Vector3d position, int octaves, double frequency, double persistence) {
            double total = 0f;
            double maxAmplifier = 0f;
            double amplitude = 1f;
            for (int i = 0; i < octaves; i++) {
                total += ((1.0 - Math.Abs(Simplex(position * frequency)))) * amplitude;
        ...
        public static double Ridged(Vector3d position, int octaves, double frequency, double persistence) {
            ...
            double amplitude = 1f;
            for (int i = 0; i < octaves; i++) {
                total += Math.Abs(Simplex(position * frequency * amplitude)) * amplitude;
